...
test_that("databaseCharacteristics works", {
  ...
  expect_no_error(databaseCharacteristics(cdm, sex = TRUE, ageGroup = list(c(0,50), c(51,Inf)), dateRange = as.Date(c("1970-01-01", NA)), conceptIdCount = TRUE) |> suppressWarnings())
  expect_no_error(databaseCharacteristics(cdm, sex = TRUE, ageGroup = list(c(0,50), c(51,Inf)), dateRange = as.Date(c("1970-01-01", NA)), conceptIdCount = TRUE, interval = "years") |> suppressWarnings())
...
test_that("shinyCharacteristics works", {
  ...
  cdm <- mockOmopSketch()
  dir <- tempdir()
  expect_no_error(result <- databaseCharacteristics(cdm, sex = TRUE, conceptIdCount = TRUE ) |> suppressWarnings())
