test_that("plotInObservation works", {
  ...
  x <- summariseInObservation(cdm$observation_period, interval = "year", unitInterval = 8)
  expect_no_error(inherits(plotInObservation(x), "ggplot"))
  x <-  x |> dplyr::filter(result_id == -1)
  expect_error(plotInObservation(x))

  expect_error(plotInObservation(summariseInObservation(cdm$observation_period, interval = "year", unitInterval = 1, output = c("person-days", "records"), ageGroup = NULL, sex = FALSE)))

  x <- summariseInObservation(cdm$observation_period, interval = "year", unitInterval = 1, output = "person-days", ageGroup = NULL, sex = FALSE)
  expect_true(inherits(plotInObservation(x), "ggplot"))

  x <- summariseInObservation(cdm$observation_period, interval = "year", unitInterval = 1, output = "records", ageGroup = NULL, sex = FALSE)
  expect_true(inherits(plotInObservation(x), "ggplot"))
