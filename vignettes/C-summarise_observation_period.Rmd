---
title: "Summarise observation periods"
output: 
  html_document:
    pandoc_args: [
      "--number-offset=1,0"
...
In this vignette, we will explore the *OmopSketch* functions designed to provide an overview of .
Specifically, there are five key functions that facilitate this:
...
-   `summariseInObservation()` and `plotInObservation()`: Use them to summarise the number of records in observation during specific intervals of time.
...
`tableObservationPeriod()` will help you to tidy the previous results and create a gt table.
...
summariseObservationPeriod(cdm$observation_period) |> 
  plotObservationPeriod(variableName = "Number subjects",
                        plotType = "barplot")
...
summariseObservationPeriod(cdm$observation_period,
                           sex = TRUE,
                           ageGroup = list("<35" = c(0, 34), ">=35" = c(35, Inf))) |> 
plotObservationPeriod(colour = "sex", 
                      facet = "age_group")
...
OmopSketch can also help you to summarise number of records in observation during specific intervals of time.
...
```{r, warning=FALSE} 
summariseInObservation(cdm$observation_period, 
                       interval = "years") |> 
  plotInObservation()
